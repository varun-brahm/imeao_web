        ...
        <?= GridView::widget([
            ...
            'columns'=>[
                ...
                [
                    ...
                    'filterType'=>GridView::FILTER_SELECT2,
                    'filterWidgetOptions'=>[
                        'data' => \yii\helpers\ArrayHelper::map(\app\models\Users::find()->orderBy('name')->all(),
                            'id', 'name'),
                        'options' => ['placeholder' => ''],
                        'pluginOptions' => ['allowClear' => true],
                    ],
                ...
                [
                    ...
                    'dropdown' => false,
                    'vAlign'=>'middle',
                     'template'=>'{edit_klas} {update_school} {studenten_add} {excel} {raw}',
                    'urlCreator' => function($action, $model, $key, $index) {
                        return Url::to(['school-'.$action,'schoolid'=>$key]);
                    },
                    ...
                    'buttons' => [
                        'edit_klas' => function ($url, $model, $key) {
                            return Html::a('<span class="fas fa-school"></span>', ['klas-update', 'klasid' => $model->klas_id],
                                [
                                    'role'=>'modal-remote',
                                    'title' => 'Klas ',
                        ...
                        'excel' => function ($url, $model, $key) {
                            return Html::a('<span class="fas fa-file-excel"></span>', ['excel-import', 'id' => $model->klas_id],
                                [
                                    'role'=>'modal-remote',
                                    'title' => 'Studenten ',
                                    'class' => 'btn btn-danger btn-xs',
                        ...
                        'raw' => function ($url, $model, $key) {
                            return Html::a('<span class="fas fa-clipboard"></span>', ['upload-raw', 'klas_id' => $model->klas_id],
                                [
                                    'role'=>'modal-remote',
                                    'title' => 'Studenten ',
                                    'class' => 'btn btn-success btn-xs',
...
//                        },
                    ...
                    'edit_klas' => function ($model, $key, $index) {
                        $user = Yii::$app->user->identity->recht_id;
                        $recht = Recht::findOne($user);
                        if ($recht->update_all == 1) {
                                return true;
                            }
                            return false;
                    },
                    'update_school' => function ($model, $key, $index) {
                        $user = Yii::$app->user->identity->recht_id;
                        $recht = Recht::findOne($user);
                        if ($recht->update_all == 1 || $recht->school_cijfer) {
                            return true;
                        }
                        return false;
                    },
                    'studenten_add' => function ($model, $key, $index) {
                        $user = Yii::$app->user->identity->recht_id;
                        $recht = Recht::findOne($user);
                        if ($recht->update_all == 1) {
                            return true;
...
//                    Html::a('<i class="fas fa-file-excel"></i>', ['excel-upload'],
//                        ['role' => 'modal-remote', 'title' => 'Nieuwe klas', 'class' => 'btn btn-success']).
                    '{toggleData}'.
                    '{export}'
                ],
