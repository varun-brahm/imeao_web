...
<!--    </div>-->
    ...
    <div class="section">
        <p>
            ...
            dat leerling <strong> <?= Html::encode($model->naam . ' ' . $model->voornaam) ?> </strong>, met identificatienummer  <strong><?= $model->id_nummer ?> </strong>,
            geboren op <strong><?= $model->geboorte_datum ?> </strong>
            en wonende aan de <strong><?= Html::encode($model->adres_student) ?> </strong>,
<!--            per <strong>--><?php //= (new DateTime($stud_info_asc->datum_inschrijving_her))->format('d/m/Y') ?><!-- </strong>-->
<!--            ingeschreven staat op eerdergenoemde school.-->
            gedurende het schooljaar <strong> <?= Html::encode($stud_info_desc->jaar->naam) ?> </strong> ingeschreven stond als leerling van <strong> <?= Html::encode($stud_info_desc->huidige_klas) ?></strong> van het IMEAO-5.
        </p>
        <p>
            Het IMEAO 5 is een mbo-school die dagonderwijs aanbiedt aan zijn leerlingen.
<!--            Leerling-->
<!--            <strong>--><?php //= Html::encode($model->naam . ' ' . $model->voornaam) ?><!--</strong>-->
<!--            zit in het schooljaar  <strong> --><?php //= Html::encode($stud_info_desc->jaar->naam) ?><!-- </strong>-->
<!--            in <strong> --><?php //= Html::encode($stud_info_desc->huidige_klas) ?><!--</strong>.-->
        </p>
    </div>
